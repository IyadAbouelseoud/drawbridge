...
> Persistent context. Read this file, `ROADMAP_ARCHIVE.md`, and `COMPLIANCE-GCC.md` at the
...
> *Fixed at the v1.0.0 freeze, four checklists later. See §23.*
...
## 23. Fail-closed, and the freeze (v1.0.0)

One defect, carried on four consecutive entry checklists as the oldest thing in the
repository, and closed last because it was structural rather than because it was hard.

### 23.1 What `neverError: true` actually did

n8n's HTTP Request node treats a non-2xx response as either a value or a failure, and
`neverError` is the switch. All fifteen calls in the three workflows had it on from week 9
to the freeze. A 500 from `/claims/persist` therefore arrived at the next node as an
ordinary item whose body was `{data: "Internal Server Error"}`.

What happened next is the part worth writing down, because it is not "the run crashed
later". The following node read a field that was not in that object, `JSON.stringify`
omitted the undefined key rather than raising, and the request it built went out **missing
a field** instead of carrying a wrong one. That is the identical failure mode as the
item-pairing defect in §20, arriving through a different door: JavaScript's willingness to
carry an absent value forward silently, meeting an API contract expressed in Pydantic that
only checks what it is given. The run continued through packaging, returned HTTP 200 to
the caller, and recorded `success`.

So the pipeline's own success signal was the thing being falsified. There is no amount of
monitoring a workflow that catches this, because every observable it emits says the run
worked. `errorWorkflow` was wired, correct, and tested — week 15 fixed it being referenced
by name instead of id — and it never fired once, because no node ever raised. An error
handler downstream of a system that cannot produce errors is decoration.

### 23.2 The fix, and the half of it that is not the flag

`neverError` is now `False` on all fifteen nodes, which is n8n's own default. It is stated
explicitly rather than omitted, because the default is exactly what was in force before
someone typed `true`, and an argued value in the artefact is something the next person has
to type over rather than fill in.

The flag is only half. `neverError` decides whether a non-2xx *is* a failure; `onError`
decides what the workflow does once it is one. Setting the first and leaving the second at
`continueRegularOutput` restores the defect through the other half of the same setting, so
the tests assert both: no node swallows a non-2xx, and no node continues past its own
failure. `onError` is absent from every node, which is n8n's `stopWorkflow`.

With both in force the shape is: the failing node raises, the run halts there,
`drawbridgeError1` fires, and the claim gets a blocking review row naming the node and the
error. An analyst sees a claim that stopped and why. Before, they saw a filing packet.

### 23.3 Why the tests assert the artefacts and not the generator

The three workflow JSONs are generated by `n8n/generate_workflows.py`, and week 12 proved
those two can disagree — the service token was hand-edited into the output and not into
the generator, and went unnoticed for three commits because nobody ran it. n8n imports the
JSON. So the fail-closed assertions read the generated files, and a separate test runs the
generator and requires the output to be byte-identical to what is committed.

That pairing is the general lesson of this repository, recorded here for the last time: a
check that passes on an artefact's *form* and never on its *use* will hold for weeks while
the thing it describes is broken. Six weeks of valid JSON that n8n could not import. A
ruling scorer that scored. An embedding text that embedded. A pipeline that ran.

### 23.4 What v1.0.0 is

A dual-jurisdiction duty recovery engine that produces a filing packet and an audit trail,
and does not file. Both lanes reproduce to the cent. Classification puts the correct
subheading in the top ten on 44 of 50 labelled queries and first on 26, and every
classification it produces against the full schedule is marked for a human — so the
recall figure is the one that describes the product and the rank-1 figure is the one that
describes the model.
