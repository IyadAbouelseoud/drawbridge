...
from services.classifier.src.embeddings import EmbeddingError, build
...
def measure(session: Session, embedder: Any, revision: str) -> dict[str, Any]:
...
def summarise(result: dict[str, Any]) -> str:
    ...
    for p in result["positives"]:
        ...
        lines.append(
            f"  rank={rank!s:>3}  d={dist}  lex={lex}  {flag}  {p['kind']:<12} {p['query'][:44]}"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    with Session(engine) as session:
        ...
        result = measure(session, embedder, args.revision)
